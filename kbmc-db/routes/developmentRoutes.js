...
router.put('/development-plan-desc/:id', (req, res) => {
    ...
    db.query(sql, [description, req.params.id], (err, result) => {
        if (err) {
            return res.status(500).json({ message: 'Failed to update development plan data', error: err });
        }
...
router.delete('/development-plan-desc/:id', (req, res) => {
    ...
    db.query(sql, [req.params.id], (err, result) => {
        if (err) {
            return res.status(500).json({ message: 'Failed to delete development plan data', error: err });
        }
        res.json({ success: true });
    });
});
